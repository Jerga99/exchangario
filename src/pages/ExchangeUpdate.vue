...
const setupInitialData = (exchange) => ({
  title: exchange?.title ?? "",
  description: exchange?.description ?? "",
  type: exchange?.type ?? "product",
...
export default {
  ...
  async created() {
    ...
    const exchange = await this.$store.dispatch("exchange/getExchangeById", id);
    if (exchange.user.id != this.authUser) {
      this.$router.push("/");
    }

    this.form = setupInitialData(exchange);
    this.params = {
      slug: exchange.slug,
      id: exchange.id 
    }
  ...
  methods: {
    async updateExchange() {
      ...
      if (isValid) {
        this.v$.$reset();
        this.$store.dispatch("exchange/updateExchange", {
          id: this.params.id,
          data: this.form,
          onSuccess: () => {
            this.$router.push(`/exchanges/${this.params.slug}`);
          }
        })
      }
    },
